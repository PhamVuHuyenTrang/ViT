...
def main(args):
    ...
    # num_workers default 10 = how many subprocesses (functions) we want for loading our data 
    # this is an interable that will be interated through in engine... iterating through what? one batch?
    ...
    # calls from torch.cuda.amp.GradScaler() in timms and provides optimisation step for model
    ...
    # label smoothing loss = less loss for wrong class, more loss for right class
    ...
    if args.eval:
        test_stats = evaluate(data_loader_val, model, device, wandb_flag=args.wandb)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        test_stats = evaluate(data_loader_val, model, device, epoch=epoch, wandb_flag=args.wandb)
        print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%")
